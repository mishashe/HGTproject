...
rule CalulateAPerCont:
	input:
		...
		mum=expand("{DATA_DIR}/processed/mummer/{{SPECIES1}}_{{SPECIES2}}",DATA_DIR=config["DATA_DIR"])
	output:
		Rdata=expand("{DATA_DIR}/processed/results/{{SPECIES1}}_{{SPECIES2}}_perCont.RData",DATA_DIR=config["DATA_DIR"])
	conda:
		config["CONDA_FILE"]
	shell:
		"""Rscript {config[CODE_DIR]}/A_per_cont.R {wildcards.SPECIES1} {wildcards.SPECIES2}"""


rule CalulateAFiltered:
	input:
		mumHM=expand("{DATA_DIR}/processed/mummerHighMem/{{SPECIES1}}_{{SPECIES2}}",DATA_DIR=config["DATA_DIR"]),
		mum=expand("{DATA_DIR}/processed/mummer/{{SPECIES1}}_{{SPECIES2}}",DATA_DIR=config["DATA_DIR"])
	output:
	...
	conda:
		config["CONDA_FILE"]
	shell:
		"""Rscript {config[CODE_DIR]}/calculate_A_from_mummer_files.R {wildcards.SPECIES1} {wildcards.SPECIES2} 1"""
