...
Table <- read.table(file="~/HGTnew/data/external/EscherichiaColi.csv",sep="\t", row.names=NULL,header=TRUE)
...
for (country in countries)
{
  ...
  ACCN <- ACCN[ACCN!=""]
  i <- 1
  write(data.frame(), paste0("~/HGTnew/data/processed/EscherichiaColi/EscherichiaColi_",country,".fasta"),append=FALSE)
  while (i<=length(ACCN))
  {
    ...
    write(seq, paste0("~/HGTnew/data/processed/EscherichiaColi/EscherichiaColi_",country,".fasta"),append=TRUE)
