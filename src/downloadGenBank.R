...
Table <- read.table(file="./data/external/EscherichiaColi.csv",sep="\t", row.names=NULL,header=TRUE)
restez_path_set("./data/external")
restez_connect()
...
for (country in countries)
{
  ...
  ACCN <- ACCN[ACCN!=""]
  i <- 1
  write(data.frame(), paste0("./data/processed/EscherichiaColi/EscherichiaColi_",country,".fasta"),append=FALSE)
  while (i<=length(ACCN))
  {
    ...
    write(seq, paste0("./data/processed/EscherichiaColi/EscherichiaColi_",country,".fasta"),append=TRUE)
